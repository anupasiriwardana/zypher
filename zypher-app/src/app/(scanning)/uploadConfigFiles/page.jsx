...
export default function UploadConfigPage() {
  ...
  const handleScan = () => {
    ...
    // API call with a random success/failure
    setTimeout(() => {
  ...
  return (
    <main className={`min-h-screen bg-background text-foreground flex flex-col pt-32 pb-16 px-4 ${lexend.className}`}>
      <div className="max-w-7xl mx-auto w-full flex flex-col md:flex-row gap-8 lg:gap-12 flex-grow"> 

        {/* LEFT SIDEBAR - File List & Upload */}
        <div className="w-full md:w-1/3 lg:w-1/4 border-[var(--border-input)] rounded-xl p-6 bg-[var(--input-bg)] shadow-xl flex flex-col">
          ...
          {files.length === 0 ? (
            ...
            <div className="flex-grow overflow-y-auto pr-2 custom-scrollbar mb-6"> {/* Custom scrollbar class */}
        ...
        <div className="flex-1 flex flex-col rounded-xl bg-[var(--input-bg)] shadow-xl p-8 items-center justify-center relative min-h-[400px]"> 
          ...
          {selectedFile && (
            <div className="flex flex-col items-center justify-center w-full h-full"> {/* Ensures content is centered within panel */}
              {scanning ? (
                <div className="flex flex-col items-center gap-6 text-[var(--brand-yellow)] animate-pulse-slow"> {/* Added pulse animation */}
                  ...
                  {scanResult === 'success' ? (
                    <div className="text-[var(--brand-yellow)]"> 
                      ...
                      <button
                        ...
                        className="bg-[var(--button-bg)] text-[var(--foreground)] border border-[var(--border-button)] font-semibold px-8 py-3 rounded-full hover:border-[var(--brand-yellow)] hover:text-[var(--brand-yellow)] transition shadow-md"
                      >
                        Scan Another File
                      </button>
                    </div>
                    ...
                    <div className="text-red-500"> 
                      ...
                      <button
                        onClick={clearSelectedFile}
                        className="bg-[var(--button-bg)] text-[var(--foreground)] border border-[var(--border-button)] font-semibold px-8 py-3 rounded-full hover:border-red-500 hover:text-red-500 transition shadow-md"
                      >
                        Try Again
                      </button>
                    </div>
                ...
                // Initial state when a file is selected but not scanned
