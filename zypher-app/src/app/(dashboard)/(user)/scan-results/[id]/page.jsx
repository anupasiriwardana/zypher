...
const ScanResult = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      setError(null); 
      try {
        const scanRes = await fetch(`/api/scan-results/${scanID}?type=${type}`, {
          method: 'GET',
          headers: {
        ...
        if (type === 'file') {
          ...
          if (processedScanData.vulnerabilityScan && processedScanData.vulnerabilityScan.findings) {
            processedScanData.vulnerabilityScan.results = groupFindingsForFileScan(
          ...
          if (processedScanData.bestPracticesScan && processedScanData.bestPracticesScan.findings) {
            processedScanData.bestPracticesScan.results = groupFindingsForFileScan(
        ...
        if (type === 'repo' && processedScanData.repo_url && processedScanData.vulnerabilityScan?.stats?.vuln_score !== undefined) {
          setInitialVulnMaxScores(prevScores => {
            if (prevScores[processedScanData.repo_url] === undefined) {
              const newScores = { ...prevScores, [processedScanData.repo_url]: processedScanData.vulnerabilityScan.stats.vuln_score };
  ...
  const renderScanTypeResults = (scanTypeData, currentScanTypeKey) => {
    ...
    if (currentScanTypeKey === 'vulnerabilityScan') {
        if (type === 'repo' && repoUrlForMaxScore && initialVulnMaxScores[repoUrlForMaxScore] !== undefined) {
    ...
    const ruleMetadataScanType = currentScanTypeKey === 'vulnerabilityScan' ? 'vulnerabilities' : 'best-practices';
      ...
      <div className="text-left animate-fadeIn">
            ...
            {displayScore !== undefined && (
                <div className="bg-[var(--background)] p-6 rounded-lg border border-[var(--border-input)] shadow-md flex flex-col items-center justify-center space-y-4 h-full">
                    <h4 className="text-md font-semibold text-[var(--text-secondary)]">
                        {currentScanTypeKey === 'vulnerabilityScan' ? 'Vulnerability Score' : 'Best Practices Score'}
                    </h4>
                    <CircularProgressBar
                        score={displayScore}
                        label="Score"
                        maxScore={currentScanTypeKey === 'vulnerabilityScan' ? vulnMaxScoreForProgressBar : 100}
                    ...
                    {currentScanTypeKey === 'bestPracticesScan' && displayScore !== undefined && (
                        <div className="flex flex-col items-center">
                            <p className="text-sm text-[var(--text-secondary)]">Overall Best Practices Rating</p>
                            {displayScore >= 90 && <span className="text-green-400 font-semibold">Excellent</span>}
                            {displayScore < 90 && displayScore >= 70 && <span className="text-yellow-400 font-semibold">Good</span>}
                            {displayScore < 70 && displayScore >= 50 && <span className="text-orange-400 font-semibold">Needs Improvement</span>}
                            {displayScore < 50 && <span className="text-red-400 font-semibold">Poor</span>}
                        </div>
                    )}
                </div>
            )}
        ...
        {results && results.length > 0 ? (
          <div className="bg-[var(--background)] p-6 rounded-xl border border-[var(--border-input)] shadow-md overflow-x-auto custom-scrollbar">
            ...
            <div className="space-y-8">
              {results.map((fileResult, fileIndex) => { 
                ...
                const isFileExpanded = expandedItems.has(`file-${fileResult.path}`);
                    ...
                    {isFileExpanded && (
                      <div className="mt-4 space-y-4 animate-fadeIn">
                        {Object.entries(groupedFindings).length > 0 ? (
                          Object.entries(groupedFindings).map(([ruleKey, ruleGroup], ruleIndex) => (
                            <div key={ruleIndex} className="p-4 rounded-md bg-[var(--background)] border border-[var(--border-input)]">
                              <button
                                ...
                                onClick={() => toggleExpansion(`rule-${fileResult.path}-${ruleKey}`)}
                              >
                                <p className="text-md font-medium text-[var(--foreground)] flex items-center gap-2 min-w-0">
                                  <Code size={16} className="text-[var(--text-secondary)]" /> Rule: <span className="break-words">{getRuleName(ruleGroup.rule_id, ruleMetadataScanType)}</span>
                                </p>
                                <span className="flex items-center gap-2">
                                    <SeverityBadge severity={ruleGroup.severity} />
                                    <span className="ml-auto shrink-0 text-[var(--text-secondary)]">
                                        {expandedItems.has(`rule-${fileResult.path}-${ruleKey}`) ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                                    </span>
                                </span>
                              </button>

                              {expandedItems.has(`rule-${fileResult.path}-${ruleKey}`) && (
                                ...
                                <div className="mt-4 space-y-3 border-t border-[var(--border-input)] pt-3 animate-fadeIn">
                                  ...
                                  {ruleGroup.occurrences.map((occurrence, occIndex) => (
          ...
          <div className="bg-[var(--background)] p-6 rounded-xl border border-[var(--border-input)] shadow-md text-center text-[var(--text-secondary)]">
            ...
            <p>Your {type === 'repo' ? 'repository' : 'file'} appears secure based on our current rules.</p>
          </div>
        )}
      </div>
  ...
  if (isLoading) {
    return (
      <div className={clsx("min-h-screen bg-[var(--background-dark)] text-white p-8 flex items-center justify-center", lexend.className)}>
        ...
        <p className="ml-3 text-lg text-[var(--text-secondary)]">Loading scan results...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className={clsx("min-h-screen bg-[var(--background-dark)] text-white p-8 flex flex-col items-center justify-center text-center", lexend.className)}>
        ...
        <h2 className="text-2xl font-bold text-red-400 mb-2">Error Loading Scan Results</h2>
  ...
  if (!scanData) {
    return (
      <div className={clsx("min-h-screen bg-[var(--background-dark)] text-white p-8 flex flex-col items-center justify-center text-center", lexend.className)}>
        ...
        <h2 className="text-2xl font-bold text-[var(--foreground)] mb-2">No Scan Data Found</h2>
        <p className="text-lg text-[var(--text-secondary)]">It seems there's no data for this scan ID or type.</p>
        <button
          onClick={() => router.back()}
          className="mt-6 px-6 py-2 bg-[var(--brand-purple)] text-white rounded-md hover:bg-[var(--brand-purple-dark)] transition-colors flex items-center gap-2"
        >
  ...
  const titleText = type === 'repo' ? (repo_url ? `Repository Scan: ${repo_url}` : 'Repository Scan') :
                    ...
                    'Scan Results';
  const subtitleText = type === 'repo' ? 'Explore vulnerabilities and best practices across your repository.' :
                       type === 'file' ? 'Review the findings for your uploaded file.' :
                       'Detailed analysis of your code scan.';

  const formattedTimestamp = createdAt ? new Date(createdAt).toLocaleString() : 'N/A'; 

  return (
    <div className={clsx("min-h-screen bg-[var(--background-dark)] text-white p-8", lexend.className)}>
      <div className="max-w-7xl mx-auto py-12">
        ...
        <h1 className="text-4xl font-extrabold text-[var(--foreground)] mb-3 leading-tight flex items-center flex-wrap">
          {type === 'repo' ? <BarChart2 className="mr-3 text-[var(--brand-yellow)]" size={36} /> : <FileText className="mr-3 text-[var(--brand-yellow)]" size={36} />}
          {titleText}
        </h1>
        <p className="text-lg text-[var(--text-secondary)] mb-6">{subtitleText}</p>

        {type === 'repo' && repo_url && (
          <div className="mb-6 flex items-center text-lg text-[var(--text-secondary)]">
            <Link2 size={20} className="mr-2 text-[var(--brand-yellow)]" />
            <a href={repo_url} target="_blank" rel="noopener noreferrer" className="text-[var(--brand-yellow)] hover:underline break-all">
              {repo_url}
            </a>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8 text-sm text-[var(--text-secondary)]">
          <div className="bg-[var(--input-bg)] p-3 rounded-lg border border-[var(--border-input)] flex items-center justify-between">
            <span>Scan ID:</span>
            <span className="font-medium text-[var(--foreground)]">{scanID}</span>
          </div>
          <div className="bg-[var(--input-bg)] p-3 rounded-lg border border-[var(--border-input)] flex items-center justify-between">
            <span>Scan Type:</span>
            <span className="font-medium text-[var(--foreground)] capitalize">{type}</span>
          </div>
          <div className="bg-[var(--input-bg)] p-3 rounded-lg border border-[var(--border-input)] flex items-center justify-between">
            <span>Scan Date:</span>
            <span className="font-medium text-[var(--foreground)]">{formattedTimestamp}</span>
          </div>
          ...
          {scan_duration && (
            <div className="bg-[var(--input-bg)] p-3 rounded-lg border border-[var(--border-input)] flex items-center justify-between">
                ...
                <span className="font-medium text-[var(--foreground)]">{scan_duration.toFixed(2)} seconds</span>
            </div>
          )}
        </div>
