...
export default function PasteUrlPageContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errorData.message ||
            "Failed to initiate scan. Please check the URL and try again."
        );
      }
      ...
      if (data.vulnerabilityScanResults || data.bestPracticesScanResults || data.customRuleScanResults) {
        ...
        setFeedback({
          type: "success",
          message: "Scan initiated successfully! Results displayed below.",
        });
        setUrl("");

        // Logic to store initial vulnerability max score
        if (
          data.vulnerabilityScanResults?.stats?.["vuln score"] !== undefined
        ) {
          setInitialVulnMaxScores((prevScores) => {
            ...
            if (currentInitialScore === undefined) {
              newScores[trimmedUrl] =
                data.vulnerabilityScanResults.stats["vuln score"];
            ...
            if (currentInitialScore === undefined) {
              newScores[trimmedUrl] =
                data.bestPracticesScanResults.stats["BSTP score"];
              // Save to localStorage immediately after updating state
              localStorage.setItem(
                "initialBpMaxScores",
                JSON.stringify(newScores)
      ...
      setFeedback({
        type: "error",
        message: error.message || "Failed to initiate scan. Please try again.",
      });
      setScanResults(null);
    } finally {
      setIsLoading(false);
  ...
  const renderScanTypeResults = (scanTypeData, scanType, repoUrl) => {
    ...
    const vulnMaxScoreForProgressBar =
      ...
      initialVulnMaxScores[repoUrl] !== undefined
        ? initialVulnMaxScores[repoUrl]
        : stats?.["vuln score"] !== undefined && stats["vuln score"] > 0
        ? stats["vuln score"]
        : 5000;

    const BpMaxScoreForProgressBar =
      scanType === "best practices" && initialBpMaxScores[repoUrl] !== undefined
        ? initialVulnMaxScores[repoUrl]
    ...
    <div className="animate-fadeInUp max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="text-center bg-[var(--input-bg)] p-8 md:p-12 rounded-3xl mb-12 shadow-xl border border-[var(--border-input)]">
        ...
        <p className="text-lg text-[var(--text-secondary)] max-w-2xl mx-auto mb-8">
          Simply paste the URL of your GitHub repository below to start a
