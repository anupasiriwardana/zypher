...
export const POST = async (request) => {
  ...
  const { ruleId, rule_name, rule_description, suggested_severity, sample_code} = await request.json();

  try {
    const customRuleRequestDoc = new UpdateRuleRequest({
      ruleId: ruleId,
      name: rule_name,
...
//       const customRuleRequests = await UpdateRuleRequest.find({ user_id: userId })
//         .sort({ createdAt: -1 })
...
//       const customRuleRequests = await UpdateRuleRequest.find()
//         .sort({ createdAt: -1 })
...
//       const customRuleRequests = await UpdateRuleRequest.find({ assigned_developer: userId })
//         .sort({ createdAt: -1 })
