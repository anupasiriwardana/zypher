...
import {
  X, Trash2, ArrowUpCircle, Hourglass, Code, FlaskConical, CheckCircle, XCircle, BookOpen
} from 'lucide-react';
...
const RuleDetailModal = ({ rule, onClose, onRemove, onUpgrade }) => {
  ...
  const handleRemove = useCallback(() => {
    if (window.confirm(`Are you sure you want to remove rule "${rule.ruleName}" from the system?`)) {
      onRemove(rule.id);
      onClose();
    }
  ...
  const handleUpgrade = useCallback(() => {
    if (window.confirm(`Are you sure you want to initiate an upgrade process for rule "${rule.ruleName}"?`)) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-[100] animate-fadeIn">
      <div className="bg-[var(--background)] rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto transform scale-95 animate-scaleIn border border-[var(--border-input)]">
        <div className="flex justify-between items-center p-6 border-b border-[var(--border-input)]">
          <h3 className="text-2xl font-bold text-[var(--foreground)]">{rule.ruleName}</h3>
        ...
        <div className="p-6 text-[var(--foreground)]">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <p className="font-medium text-[var(--brand-yellow)]">{rule.id}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium">{rule.targetFileTypes || 'N/A'}</p>
            </div>
            <div>
              <p className="text-sm text-[var(--text-secondary)]">Approved Date:</p>
              {/* Assuming submittedDate is also the approved date for simplicity or add a new field */}
              <p className="font-medium">{new Date(rule.submittedDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
            </div>
          </div>

          <div className="mb-6">
            <p className="text-sm text-[var(--text-secondary)] mb-2">Description:</p>
            <p className="bg-[var(--input-bg)] p-3 rounded-md text-sm border border-[var(--border-input)]">{rule.description}</p>
          </div>

          {rule.developedCode && (
            <div className="mb-6">
              <p className="text-sm text-[var(--text-secondary)] mb-2">Developed Code:</p>
              <pre className="bg-[var(--input-bg)] p-3 rounded-md text-xs overflow-x-auto border border-[var(--border-input)]">
                <code className="text-[var(--foreground)]">{rule.developedCode}</code>
              </pre>
            </div>
          )}
          ...
          {rule.testDetails && (
            <div className="mb-6">
              <p className="text-sm text-[var(--text-secondary)] mb-2">Test Details:</p>
              <p className="bg-[var(--input-bg)] p-3 rounded-md text-sm border border-[var(--border-input)] text-purple-300/80">{rule.testDetails}</p>
            </div>
          )}
