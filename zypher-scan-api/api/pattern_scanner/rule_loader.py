...
import os
import types
from config.database import security_pattern_rule_collection
from models.pattern import Finding
from pattern_scanner.baseRule_class import BaseRule

def load_rules() -> list:
    """Load vulnerability rules from MongoDB and instantiate them dynamically."""
    rules = []
    active_rules = security_pattern_rule_collection.find({"status": "active"})
